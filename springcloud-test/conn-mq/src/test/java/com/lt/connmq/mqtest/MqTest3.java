...
@SpringBootTest
class MqTest3 {
    Logger logger = LoggerFactory.getLogger(MqTest3.class);
    public static final String DEAD_EXCHANGE = "dead_exchange_3";
    public static final String DEAD_QUEUE = "dead_queue_3";
    public static final String DEAD_ROUT_KEY = "dead_route_3";

    public static final String NORMAL_EXCHANGE = "normal_exchange_3";
    public static final String NORMAL_QUEUE = "normal_queue_3";
    public static final String NORMAL_ROUT_KEY = "normal_route_3";
    ...
    void deadConsumer01Test() throws Exception {
        Channel channel = RabbitmqUtil.getChannel(false);
        logger.info("deadConsumer01Test  等待接收消息~~~");
        DeliverCallback deliverCallback = (v1, v2) -> {
            String msg = new String(v2.getBody(), Charset.defaultCharset());
            if (msg.startsWith("info:5")) {
                logger.info("vonsumer 1 接收到的消息为：[{}],拒收", msg);
                channel.basicReject(v2.getEnvelope().getDeliveryTag(), false);
            } else {
                logger.info("vonsumer 1 接收到的消息为：[{}],消费了", msg);
                channel.basicAck(v2.getEnvelope().getDeliveryTag(), false);
            }

        };
        //设置自动确认消息为false  需要手动确认消息
    ...
    void deadProduceTest() throws Exception {
        ...
        AMQP.BasicProperties build = new AMQP.BasicProperties()
                ...
                .build();
        for (int i = 0; i < 11; i++) {
            String msg = "info:" + i + ":" + UUID.randomUUID();
